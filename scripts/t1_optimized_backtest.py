...
from engine.strategies.t1_optimized import T1Optimized
from engine.t1_optimized_sell import T1OptimizedSellEngine
...
def run_backtest(
    ...
    # 初始化策略和卖出引擎
    strategy = T1Optimized()
    sell_engine = T1OptimizedSellEngine()
...
def generate_report(trades: list, sell_engine: T1OptimizedSellEngine):
    """生成回测报告"""
    if not trades:
        print("无交易记录")
        return
    ...
    # 指标分析（T1Optimized只有3个条件）
    print(f"\n选股指标统计:")
    if "upper_shadow" in df_trades.columns:
        print(f"  平均上影线: {df_trades['upper_shadow'].mean():.2f}%")
    if "pct_chg" in df_trades.columns:
        print(f"  平均当日涨幅: {df_trades['pct_chg'].mean():.2f}%")

    # 保存详细交易记录
    output_path = project_root / "data" / "backtest_results" / "t1_optimized_trades.csv"
