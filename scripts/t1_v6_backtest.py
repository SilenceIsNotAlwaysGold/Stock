...
from engine.strategies.t1_v6 import T1V6LowRebound
from engine.t1_v6_sell import T1V6SellEngine
...
def run_backtest(
    ...
    # 初始化策略和卖出引擎
    strategy = T1V6LowRebound()
    sell_engine = T1V6SellEngine()
...
def generate_report(trades: list, sell_engine: T1V6SellEngine):
    """生成回测报告"""
    if not trades:
        print("无交易记录")
        return
    ...
    # 保存详细交易记录
    output_path = project_root / "data" / "backtest_results" / "t1_v6_trades.csv"
