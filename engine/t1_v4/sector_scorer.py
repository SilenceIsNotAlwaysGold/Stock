...
满分 17 分。隔夜策略的核心逻辑是"强者恒强"，
热门板块的股票次日高开概率显著高于冷门板块。
A股板块轮动效应显著，板块面对短线选股至关重要。
"""
...
class SectorScorer:
    ...
    def score(
        ...
        Returns:
            {
                "sector_total": float,          # 总分 0-17
                "rank_score": float,            # 行业排名 0-9
                "limit_up_score": float,        # 涨停数 0-5
    ...
    def _score_rank(
        ...
        连续映射（排名越靠前分越高）：
          rank_pct 0%  (最强): 9 分
          rank_pct 20%        : 9 分（前 20% 都满分）
          rank_pct 50%        : 3.4 分
          rank_pct 80%        : 0.6 分
          rank_pct 100% (最弱): 0 分
        sector_rank 或 total_sectors 为 None 时返回 4.5 分（中间值）
        """
        ...
        fp = [9.0, 9.0, 3.4, 0.6, 0.0]

        score = float(np.interp(rank_pct, xp, fp))
        return float(np.clip(score, 0.0, 9.0))
